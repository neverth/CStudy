...
private:
	int year, month, day;
public :
	Date(int con_year = 0, int con_month = 0, int con_day = 0):
...
public:
	Bird(int fh  = 0):m_nFlightAltitude(fh){
		cout << "bird 构造函数" << endl;
	}
 	void fly_in_shy() {
...
int main() {
	...
	//解决办法二 派生类中定义与基类同名的成员函数， 将基类函数隐藏
	waterbird.breath();
	...
	Bird *p1 = new WaterBird;
	p1->breath();
	Fish *p2 = new WaterBird;
	p2->breath();
	(new WaterBird)->breath();
	//system("pause");
	return 0;
}